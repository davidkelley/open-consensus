...
- **SUPERSEDES D-PKG9 + Stage 4 + the openconsensus.dev assumption — the installer is hosted as a GitHub Release ASSET, not a Cloudflare Worker on a custom domain** (user decision, post-build, to drop the domain cost). `install.sh` (at the repo root) is uploaded as a release asset by `release.yml`; the install command is `curl -fsSL https://github.com/davidkelley/open-consensus/releases/latest/download/install.sh | sh`. This removes the entire `apps/installer/` Cloudflare worker (and the isolated-sub-project deviation it required), the `openconsensus.dev` purchase + Cloudflare zone prerequisite, and the worker's `?version=` URL templating (pinning is via the `OPEN_CONSENSUS_VERSION` env var, which the script already supports). The only remaining release prerequisite is making the GitHub repo public with Releases. CI now shellchecks `install.sh` in the gate job (the worker vitest job is gone).
